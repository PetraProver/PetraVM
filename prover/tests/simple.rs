...
use binius_field::{BinaryField32b, BinaryField};
use zcrayvm_assembly::{Opcode, LDIEvent, RetEvent};
...
/// 
/// # Returns
/// * A ZkVMTrace containing an LDI instruction that loads `value` into register 2,
...
fn generate_ldi_ret_trace(value: u32) -> ZkVMTrace {
    ...
    // LDI r2, value - Load immediate value into register 2
...
fn test_trace_generation() -> Result<()> {
    ...
    assert_eq!(trace.program.len(), 2, "Program should have exactly 2 instructions");
    
    // Verify first instruction is LDI
    assert_eq!(trace.program[0].opcode, Opcode::Ldi, "First instruction should be LDI");
    
    // Verify second instruction is RET
    assert_eq!(trace.program[1].opcode, Opcode::Ret, "Second instruction should be RET");
    ...
    assert_eq!(dst, 2, "LDI destination register should be 2");
    assert_eq!(imm_low, (value & 0xFFFF) as u16, "LDI imm_low should match lower 16 bits");
    assert_eq!(imm_high, (value >> 16) as u16, "LDI imm_high should match upper 16 bits");
    
    // Verify exactly one LDI event
    assert_eq!(trace.ldi_events().len(), 1, "Should have exactly one LDI event");
    
    // Verify exactly one RET event
    assert_eq!(trace.ret_events().len(), 1, "Should have exactly one RET event");
    
    // Verify LDI event has correct value
    assert_eq!(trace.ldi_events()[0].imm, value, "LDI event should have correct immediate value");
    
    // Verify RET event has correct destination PC and FP
    assert_eq!(trace.ret_events()[0].fp_0_val, 0, "RET event should return to PC=0");
    assert_eq!(trace.ret_events()[0].fp_1_val, 0, "RET event should return to FP=0");
...
fn test_trace_validation() -> Result<()> {
    ...
    assert!(trace.validate().is_ok(), "Valid trace should pass validation");
    
    // Create an invalid trace with no instructions
    let invalid_trace = ZkVMTrace::new();
    
    // Should fail validation
    assert!(invalid_trace.validate().is_err(), "Empty trace should fail validation");
