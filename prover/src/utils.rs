...
/// Sets up a multiplexer constraint for bit selection.
///
/// Creates a constraint that enforces: result = select_bit ? when_true :
/// when_false
///
/// # Arguments
///
/// * `table` - The table builder to add the constraint to
/// * `result` - The result column that will store the selected value
/// * `when_true` - The column to select when select_bit is 1
/// * `when_false` - The column to select when select_bit is 0
/// * `select_bit` - The control bit that determines which value to select
pub fn setup_mux_constraint<const N: usize>(
    table: &mut TableBuilder,
    result: &Col<B1, N>,
    when_true: &Col<B1, N>,
    when_false: &Col<B1, N>,
    ...
    // Create packed versions of columns
