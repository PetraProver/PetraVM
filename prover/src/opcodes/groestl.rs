...
impl Table for Groestl256CompressTable {
    ...
    fn new(cs: &mut ConstraintSystem, channels: &Channels) -> Self {
        ...
        //         src1_addresses[i],
        //         src1_vals_packed[i],
        //         format!("groestl_compress_src1_val_{}", i).as_str(),
        //     )
        // });
        ...
        //         src2_addresses[i],
        //         src2_vals_packed[i],
        //         format!("groestl_compress_src2_val_{}", i).as_str(),
        //     )
        // });
        ...
        let dst_lookups = from_fn(|i| {
            MultipleLookupColumns::new(
                ...
                dst_addresses[i],
                out_packed[i],
                format!("groestl_compress_dst_val_{}", i).as_str(),
            )
        });
    ...
    fn fill<'a>(
...

        {
            ...
            for (i, event) in rows.clone().enumerate() {
                ...
                println!("src2 val {:?}", event.src2_val);
                println!("src2 u32: {:?}", src2_u32);
                for j in 0..16 {
                    src1_selected[j][i] = src1_u32[j];
                    src2_selected[j][i] = src2_u32[j];
                }

                println!("src2_selected: [{:?}]", src2_selected);
                ...
                for j in 0..8 {
                    ...
                    interm[j][i] = p_out[j]
                        .iter()
                        .zip(event.src1_val[j * 8..(j + 1) * 8].iter())
                        .map(|(&a, &b)| a.val() ^ b)
                        .collect::<Vec<_>>()
                        .try_into()
                        .unwrap();
        ...
        //     .iter()
        //     .enumerate()
        //     .map(|(i, src1_lookup)| src1_lookup.populate(witness,
        // self.src2_lookups
        //     .iter()
        //     .enumerate()
        //     .map(|(i, src2_lookup)| {
        //         src2_lookup.populate(witness, src2_lookup_iters[i].clone())
        //     })
        //     .collect::<Result<(), _>>()?;
        ...
        let q_states = rows
            .clone()
            .map(|event| {
                let mut q_state = [B8::ZERO; 64];
                for i in 0..64 {
                    q_state[i] = B8::from(event.src2_val[i]);
                }
                q_state
            })
            .collect::<Vec<_>>();
        ...
        self.p_op.populate(witness)?;
        {
            let p_out = self
                .p_op
                .state_out()
                .map(|state_out_i| witness.get(state_out_i));
            println!("p_out {:?}", p_out);
        }
...
impl Table for Groestl256OutputTable {
    ...
    fn new(cs: &mut ConstraintSystem, channels: &Channels) -> Self {
        ...
        let src2_vals_packed =
            from_fn(|i| table.add_packed(format!("src2_val_{}", i), src2_vals[i]));
    ...
    fn fill<'a>(
        ...
        }
        {
            let src2_vals_packed = (0..4)
                .map(|i| witness.get_mut(self.src2_vals_packed[i]))
                .collect::<Result<Vec<_>, _>>()?;
            println!("src2_vals_packed: {:?}", src2_vals_packed);
        }
