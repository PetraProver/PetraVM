...
fn test_calli() {
    ...
    // Add the target PC manually to the instruction, since we don't have access to the actual
    // compiler that would normally do this
    let target_addr = pc3;
    ...
    assert!(trace.calli.len() > 0, "CALLI event should have been generated");
