...
macro_rules! impl_binary_operation {
    ($t:ty) => {
        ...
        impl $crate::event::model::binary_ops::NonImmediateBinaryOperation for $t {
            fn new(
                timestamp: u32,
                pc: BinaryField32b,
                fp: u32,
...
macro_rules! impl_left_right_output_for_imm_bin_op {
    ($t:ty, $imm_field_ty:ty) => {
        impl $crate::event::model::binary_ops::LeftOp for $t {
            type Left = BinaryField32b;
            fn left(&self) -> BinaryField32b {
                BinaryField32b::new(self.src_val)
            }
        }
        impl $crate::event::model::binary_ops::RightOp for $t {
        ...
        impl $crate::event::model::binary_ops::OutputOp for $t {
    ...
    ($ty:ty) => {
        impl $crate::event::model::Event for $ty {
            fn fire(
                ...
                use $crate::event::model::binary_ops::{LeftOp, OutputOp, RightOp};
...
macro_rules! impl_immediate_binary_operation {
    ($t:ty) => {
        ...
        impl $crate::event::model::binary_ops::ImmediateBinaryOperation for $t {
            fn new(
                timestamp: u32,
                pc: BinaryField32b,
                fp: u32,
