use std::{collections::HashMap, error::Error, hash::Hash, ops::{Index, IndexMut}};
...
impl Interpreter {
    ...
    pub fn step(&mut self, trace: &mut ZCrayTrace) -> Result<Option<()>, InterpreterError>{
        let Instruction {opcode, src1, src2, dst} = &self.prom[self.pc as usize];
        match opcode {
            Opcode::Bnz => {
...
mod tests {
    ...
    #[test]
    fn test_zcray() {
        let trace = ZCrayTrace::generate(ProgramRom(vec![Instruction { opcode: Opcode::Bnz, src1: 0, src2: 0, dst: 0}])).expect("Ocuh!");
        trace.validate();
    }
}
